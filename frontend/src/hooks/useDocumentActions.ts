...
export function useDocumentActions({
  ...
  const [updatingTagsDocs, setUpdatingTagsDocs] = useState<
    ...
    async (fileId: string) => {
      ...
      try {
        const response = await fetch(
          `/api/documents/${fileId}?organizationId=${organizationId}`,
          {
            method: "DELETE",
          }
        );
            ...
            ? [
                {
                  ...
                  confidence: result.data.documentTag?.confidence || 1.0,
                },
              ]
            : tags.map((tag) => ({ name: tag, confidence: 1.0 })));
