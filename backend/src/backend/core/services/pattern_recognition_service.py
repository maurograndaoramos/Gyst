...
class PatternRecognitionService:
    ...
    async def analyze_document_with_context(
        ...
        async with self.processing_semaphore:
            try:
                ...
                logger.info(f"Starting contextual analysis for {request.document_path} (request_id: {request_id})")
                ...
                if request.context_document_paths:
                    context_tools = await self._prepare_context_tools(request.context_document_paths)
                ...
                tags = await self._parse_pattern_recognition_result(result, request.max_tags)
                ...
                logger.info(f"Contextual analysis completed for {request.document_path} in {processing_time:.2f}s (request_id: {request_id})")
    ...
    async def _prepare_context_tools(self, context_document_paths: List[str]) -> List[Any]:
        ...
        for doc_path in context_document_paths:
            try:
                ...
                logger.warning(f"Failed to create tool for context document {doc_path}: {str(e)}")
                continue
    ...
    async def _parse_pattern_recognition_result(self, result: Any, max_tags: int) -> List[TagModel]:
    ...
    async def _generate_pattern_summary(self, file_path: str, tags: List[TagModel]) -> str:
        ...
        try:
            ...
            crew = Crew(
                agents=[self.text_pattern_agent],
                tasks=[summary_task],
                verbose=False
            )
