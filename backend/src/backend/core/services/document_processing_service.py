...
class DocumentProcessingService:
    ...
    async def process_single_document(
        ...
        try:
            ...
            # Optimize chunks
            optimized_chunks, optimization_metrics = (
                await self.optimizer.optimize_chunks(chunks, document_path)
            )
            ...
            logger.info(
                f"Successfully processed {document_path}: {len(optimized_chunks)} chunks"
            )
    ...
    async def process_document_batch(
        ...
        processing_strategy = self.document_factory.get_processing_strategy(
            document_paths
        )
        ...
        for file_ext, doc_group in processing_strategy.items():
            ...
            logger.info(
                f"Processing {len(doc_group)} {file_ext} files with concurrency {effective_limit}"
            )
        ...
        average_quality = (
            sum(r.extraction_quality for r in successful_results)
        ...
        try:
            ...
            if compare_with_existing and new_result.success:
                ...
                comparison_data["comparison"] = {
                    "chunk_count_diff": new_result.chunk_count
                    - adaptive_result.chunk_count,
                    "quality_diff": new_result.extraction_quality
                    - adaptive_result.extraction_quality,
                    "time_diff": new_result.processing_time
                    - adaptive_result.processing_time,
                    "recommended_strategy": (
                        new_strategy.value
                        if new_result.chunk_count > adaptive_result.chunk_count
                        and new_result.extraction_quality
                        >= adaptive_result.extraction_quality
                        else "adaptive"
                    ),
                }
    ...
    async def _generate_embeddings_for_chunks(
        self, chunks: List[Any], document_path: str
    ) -> int:
        ...
        try:
            ...
            batch_result = await self.embedding_service.generate_batch_embeddings(
                contents=chunk_contents, content_types=chunk_types, use_cache=True
            )
        ...
        if success:
            ...
            if quality_score > 0:
                ...
                self._stats["average_quality_score"] = (
                    current_avg * (successful_docs - 1) + quality_score
                ) / successful_docs
        else:
    ...
    def get_processing_summary(self) -> Dict[str, Any]:
        ...
        avg_time_per_doc = (
            self._stats["total_processing_time"] / total_docs if total_docs > 0 else 0.0
        )

        success_rate = (
            self._stats["successful_documents"] / total_docs if total_docs > 0 else 0.0
        )

        avg_chunks_per_doc = (
            self._stats["total_chunks_created"] / self._stats["successful_documents"]
            if self._stats["successful_documents"] > 0
            else 0.0
        )
        ...
        return {
            ...
            "configuration": {
                ...
                "supported_extensions": list(
                    self.document_factory.get_supported_extensions()
                ),
            },
            "component_status": {
                "chunker": "healthy",
    ...
    async def analyze_document(
        ...
        try:
            ...
            logger.info(
                f"CrewAI analysis completed for {document_path} with {len(analysis_result.tags)} tags"
            )
